...
const sendEmail = async (to, subject, text) => {
  const msg = {
    to,
    from: "the.guide.student@mitaoe.ac.in", // Change to your verified sender
    subject,
    text,
...
exports.signupUser = async (req, res) => {
  ...
  try {
    ...
    if (
      email.split("@")[1] !== "mitaoe.ac.in"
    ) {
      return res.status(401).json({ message: "Email should be a MIT email" });
    }
    ...
    user.OTP = randomOTP;
    if (email.slice(-11) === "@mitaoe.ac.in") user.isAdmin = true;
